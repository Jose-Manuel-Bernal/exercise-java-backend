...
import com.sofka.exercise.java_backend.tournament.usecases.cyclist.GetCyclistsByCountryUseCase;
...
public class GetCyclistsByCountryRoute {
    ...
    public RouterFunction<ServerResponse> getCyclistsByCountry(GetCyclistsByCountryUseCase useCase) {
        return route(GET("/cyclist/getByCountry/{country}")
                ,request -> ServerResponse.ok().contentType(MediaType.APPLICATION_JSON)
                        .body(BodyInserters.fromProducer(useCase.apply(request.pathVariable("country")), CyclistDTO.class))
